...
base='http://lexdan.co'
ADDON=xbmcaddon.Addon(id='plugin.video.lexdan')
...
def CATEGORIES():
    link = OPEN_URL('http://lexdan.co/wizard.txt').replace('\n','').replace('\r','')
...
def wizard(name,url,description):
    ...
    dp.create("Selected Build","Baixando ",'', 'Por Favor Espere')
...
def killxbmc():
    choice = xbmcgui.Dialog().yesno('Desligando o Kodi', 'Desligando o Kodi', 'Vc quer continuar ?', nolabel='Nao, Cancela',yeslabel='Sim, Desligar')
    if choice == 0:
        return
    elif choice == 1:
        pass
