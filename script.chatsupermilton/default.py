...
class irc_client(threading.Thread):
        ...
        try:
            ...
            if 'getaddrinfo failed' in f_exc:
                xbmc.executebuiltin("XBMC.Notification(%s, %s,10000,%s)"
                    %('ChatSuperMilton', '%s: %s' %(language(32034), self.host), icon))
            else:
                addon_log(str(f_exc))
            return False
    ...
    def login(self, **kwargs):
        ...
        try:
            ...
            chat_queue.put(('ChatSuperMilton %s' %language(32024), str(format_exc())))
            return False
...
class GUI(xbmcgui.WindowXMLDialog):
    ...
    def onInit(self):
        ...
        self.client.start()
        self.window.setProperty('windowLabel', 'ChatSuperMilton')
        self.channels_list = None
        chat_queue.put(('ChatSuperMilton', language(32021)))
    ...
    def connect_to_server(self):
        ...
        if self.window.getProperty('connected') != "True":
            ...
            if not connect:
                chat_queue.put(('ChatSuperMilton', language(32034)))
            else:
                self.window.setProperty('windowLabel', self.host)
                chat_queue.put(('ChatSuperMilton', language(32017)))
                ...
                if logged_in:
                    ...
                    chat_queue.put(('ChatSuperMilton', '%s: %s' %(language(32018), self.host)))
                ...
                if not logged_in:
                    chat_queue.put(('ChatSuperMilton', language(32019)))
                elif self.channel:
            ...
            self.window.setProperty('windowLabel', 'ChatSuperMilton')
            chat_queue.put(('ChatSuperMilton', language(32020)))
    ...
    def join_channel(self):
        ...
        if self.window.getProperty('inChannel') != 'True':
            ...
            chat_queue.put(('ChatSuperMilton', '%s: %s' %(language(32022), self.channel)))
            try:
                ...
                try:
                    ...
                    addon_log('addonException: %s' %print_exc())
                    chat_queue.put(('ChatSuperMilton %s' %language(32024), str(print_exc())))
                    xbmc.executebuiltin("Skin.Reset(ChatIsLoading)")
                    return
            ...
            self.window.setProperty('windowLabel', self.host)
            client_queue.put('client_wait')
            chat_queue.put(('ChatSuperMilton', '%s %s' %(language(32023), self.channel)))
            while not client_queue.empty():
            ...
            for i in range(0,10):
                if self.window.getProperty('inChannel') == 'True':
                    ...
                    chat_queue.put(('ChatSuperMilton', 'Quit: %s' %self.channel))
            ...
            if user != self.nickname:
                if index == 1:
                    ...
                    for i in whois_info.keys():
                        whois_str += '%s: %s \n' %(i, whois_info[i])
                    chat_queue.put(('ChatSuperMilton', whois_str))
                    client_queue.put('client_run')
                    xbmc.executebuiltin("Skin.Reset(ChatIsLoading)")
            else:
                if index == 1:
...
def check_args():
    try:
        ...
        xbmc.executebuiltin("XBMC.Notification(%s, %s,10000,%s)"
            %('ChatSuperMilton', language(32046), icon))
        addon.openSettings()
        return False
